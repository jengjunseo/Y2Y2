...
export async function GET(request) {
  ...
  if (url.searchParams.get('action') === 'health') {
    return json({ ok: true, service: 'y2y2-direct-edge-resolver', version: '1.1.0-experimental', runtime: 'edge', mediaProxy: false });
  }
...
export async function POST(request) {
  ...
  for (const profile of CLIENT_PROFILES) {
    try {
      ...
      return json({
        ok: true, mode: 'direct', runtime: 'edge', videoId, title,
        author: data?.videoDetails?.author || null,
        duration: Number(data?.videoDetails?.lengthSeconds || 0),
        resolverClient: profile.key,
        progressive: formats.progressive.map(format => ({ ...format, downloadUrl: withDownloadTitle(format.directUrl, title) })),
  ...
  return json({
    error: botCheck ? 'YouTube가 현재 Edge Resolver 요청을 bot-check로 차단했습니다.' : '직접 다운로드 가능한 progressive 스트림을 찾지 못했습니다.',
    code: botCheck ? 'UPSTREAM_BOT_CHECK' : 'NO_DIRECT_FORMAT', attempts
  }, botCheck ? 502 : 422);
}
